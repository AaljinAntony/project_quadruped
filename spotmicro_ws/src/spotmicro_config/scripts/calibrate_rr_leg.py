...
class LegCalibrationClient(Node):
    def __init__(self):
        ...
        # Target Leg Motors
        self.motors = [
            {'name': 'RR Foot', 'pin': 12, 'start_pwm': 244},
            {'name': 'RR Leg', 'pin': 13, 'start_pwm': 328},
            {'name': 'RR Shoulder', 'pin': 14, 'start_pwm': 320}
        ]
        self.current_idx = 0
        self.current_pwm = self.motors[self.current_idx]['start_pwm']
        self.last_status = 'Waiting for ESP32...'
        self.log_file = '/workspace/rr_leg_calibration.log'
...
def main():
    try:
        ...
        print('\nSpotMicro RR Leg Calibration (Pins 12, 13, 14)')
        ...
        print('\n\nExited. Logs saved to /workspace/rr_leg_calibration.log.')
        if 'node' in locals():
