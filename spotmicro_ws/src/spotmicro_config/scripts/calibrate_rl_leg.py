...
class LegCalibrationClient(Node):
    def __init__(self):
        ...
        # Target Leg Motors
        self.motors = [
            {'name': 'RL Foot', 'pin': 4, 'start_pwm': 370},
            {'name': 'RL Leg', 'pin': 5, 'start_pwm': 285},
            {'name': 'RL Shoulder', 'pin': 6, 'start_pwm': 293}
        ]
        self.current_idx = 0
        self.current_pwm = self.motors[self.current_idx]['start_pwm']
        self.last_status = 'Waiting for ESP32...'
        self.log_file = '/workspace/rl_leg_calibration.log'
...
def main():
    try:
        ...
        print('\nSpotMicro RL Leg Calibration (Pins 4, 5, 6)')
        ...
        print('\n\nExited. Logs saved to /workspace/rl_leg_calibration.log.')
        if 'node' in locals():
