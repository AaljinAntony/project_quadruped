...
// Conversion factor: 115 ticks per Rad approx 2.007 ticks per Degree
const float TICKS_PER_DEG = 115.0 / 57.2958;
const float TICKS_PER_RAD = 115.0;
const int PWM_CENTER = 307;

// --- TRANSITION GLOBALS ---
float current_pwm[16];
float target_pwm[16];
float start_pwm_transition[16];
unsigned long transition_start_time = 0;
unsigned long last_transition_update = 0;
bool is_transitioning = false;
const float TRANSITION_DURATION_MS = 2000.0; // 2.0 seconds span
...
// --- UTILITIES ---
int angleToPwmTick(float angle_units, float direction, bool is_rad) {
  float ticks_offset = angle_units * (is_rad ? TICKS_PER_RAD : TICKS_PER_DEG) * direction;
  return PWM_CENTER + (int)ticks_offset;
}
...
void set_pose(int pose_id) {
    ...
    for(int i=0; i<16; i++) {
        start_pwm_transition[i] = current_pwm[i];
    }

    if (pose_id == 0) { // Neutral (Hardware Center)
        for(int i=0; i<16; i++) target_pwm[i] = PWM_CENTER;
    } else if (pose_id == 1) { // Calibrated Stand
        for(int i=0; i<12; i++) {
            int offset_pwm = PWM_CENTER + (int)(joint_offsets_deg[joint_mapping[i].offset_index] * TICKS_PER_DEG);
            target_pwm[joint_mapping[i].pca_pin] = offset_pwm;
        }
    } else if (pose_id == 2) { // Calibrated Sit (Legs up 30°, Feet in 30°)
        for(int i=0; i<12; i++) {
            int base_idx = joint_mapping[i].offset_index;
            int type = base_idx % 3; // 2=Shoulder, 1=Leg, 0=Foot
            float angle_deg = joint_offsets_deg[base_idx];
            float dir = joint_mapping[i].direction;
            
            if (type == 1) angle_deg += (30.0 * dir); // Leg up
            if (type == 0) angle_deg -= (30.0 * dir); // Foot in
            
            int final_pwm = PWM_CENTER + (int)(angle_deg * TICKS_PER_DEG);
            target_pwm[joint_mapping[i].pca_pin] = final_pwm;
        }
    }
}
    ...
    for (size_t i = 0; i < msg->name.size; i++) {
        for (int j = 0; j < 12; j++) {
            if (strcmp(msg->name.data[i].data, joint_mapping[j].name) == 0) {
                ...
                float angle_deg_relative = angle_rad * 57.2958;
...
void calib_callback(const void * msgin) {
    ...
    if (msg->data.size >= 2) {
        ...
        if (pin >= 0 && pin < 16) {
            ...
            current_pwm[pin] = pwm_val;
            is_transitioning = false; // Interrupted by calibration override
...
void setup() {
  ...
  for(int i=0; i<16; i++) {
      current_pwm[i] = PWM_CENTER;
      target_pwm[i] = PWM_CENTER;
      start_pwm_transition[i] = PWM_CENTER;
  }

  // Default to Stand on power-up (transitions over 2s automatically)
  set_pose(1);
...
void loop() {
  ...
  if (is_transitioning) {
      ...
      // Update at max 50Hz (every 20ms) to prevent I2C bus flooding
      if (now - last_transition_update >= 20) {
          last_transition_update = now;
          
          float t = (now - transition_start_time) / TRANSITION_DURATION_MS;
          if (t >= 1.0) {
              t = 1.0;
              is_transitioning = false;
          }
          
          // Apply Smoothstep (Ease-in, Ease-out) formula for natural robotic motion
          float progress = t * t * (3.0f - 2.0f * t);
          
          for(int i=0; i<12; i++) { 
              int pin = joint_mapping[i].pca_pin;
              int new_pwm = (int)(start_pwm_transition[pin] + (target_pwm[pin] - start_pwm_transition[pin]) * progress);
              
              // Only command the PCA9685 chip if the PWM value explicitly changed
              if (new_pwm != (int)current_pwm[pin] || t >= 1.0) {
                  current_pwm[pin] = new_pwm;
                  pwm.setPWM(pin, 0, new_pwm);
              }
          }
      }
  }
