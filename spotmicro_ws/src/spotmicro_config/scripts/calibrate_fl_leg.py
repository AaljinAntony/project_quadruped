...
class LegCalibrationClient(Node):
    def __init__(self):
        ...
        # Target Leg Motors
        self.motors = [
            {'name': 'FL Foot', 'pin': 0, 'start_pwm': 370},
            {'name': 'FL Leg', 'pin': 1, 'start_pwm': 297},
            {'name': 'FL Shoulder', 'pin': 2, 'start_pwm': 297}
        ]
        self.current_idx = 0
        self.current_pwm = self.motors[self.current_idx]['start_pwm']
        self.last_status = 'Waiting for ESP32...'
        self.log_file = '/workspace/fl_leg_calibration.log'
...
def main():
    try:
        ...
        print('\nSpotMicro FL Leg Calibration (Pins 0, 1, 2)')
        ...
        print('\n\nExited. Logs saved to /workspace/fl_leg_calibration.log.')
        if 'node' in locals():
